...
message = sprintf(['This GUI is operated by keyboard shortcuts. All shortcuts are executed as callbacks of the spectrogram window so if something doesn''t work it may be because the key was pressed while another window was selected\n' ...
    ...
    '\nu - [u]pdate: update parameters if they were changed in the parameters window. Update threshold or time window if they were changed, recalculate threshold boundaries (green and red lines)' ...
    '\ne - [e]rase: erase file entry (not the file). Currently this is irreversible and a prompt will pop up' ...
    '\nr - [r]ecolor: update colors in threshold and time window panels' ...
    '\nt - [t]ag: apply tag to selected syllable .. as chosen in the parameters window' ...
    '\np - [p]lay: play current segment' ...
    '\ns - label [s]elected: select segment on the spectrogram window in which all syllables will receive the selected label' ...
    '\nd - [d]elete: delete selected syllable' ...
    '\nm - [m]agnify: select a segment in the spectrogram to focus on' ...
    '\ng - [g]roup move: move all visible syllable edges to nearest threshold crossing (same as double clicking all syllables) if no overlaps occur' ...
    '\nf - group move only left edges' ...
    '\nh - group move only right edges' ...
    '\nj - join: join the current selected syllable with the next and apply the selected syllable''s tag' ...
    '\nb - [b]atch create: create new syllables at the current threshold boundaries (green and red lines) and give them the selected tag' ...
    '\nn - [n]ew: save and open the next file entry (or the selected entry if it''s not the next in the list)' ...
    '\nq - quit']);
msgbox(message,'HELP')
